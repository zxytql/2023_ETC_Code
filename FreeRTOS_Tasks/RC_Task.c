...
//    }
		if(value->key_c != 0)
		{
				if(event->key_c_middle_now == 0)
				{
					event->key_c_middle_now = 1;
					event->key_c_middle_flag = 1;
					

					Rc_Key_c_middle_Callback();
				}
				else if(event->key_c_down_now == 0)
				{
					event->key_c_down_now = 1;
					event->key_c_down_flag = 1;		
							
					Rc_Key_c_down_Callback();					
				}
		}
		else
		{
			event->key_c_middle_now = 0;
			event->key_c_down_now = 0;
		}
}
...
void Rc_Key_Callback_Handler(rc_key_event_t *event)
{
    ...
    if(event->key_d_flag == 1)
    {
		...
		if(event->key_c_middle_flag == 1)
		{
				Rc_Key_c_middle_Callback();
				
				event->key_c_middle_flag = 0;
		}
		if(event->key_c_down_flag == 1)
		{
				Rc_Key_c_down_Callback();
				event->key_c_down_flag = 0;
		}
}
...
void Rc_Key_c_down_Callback(void)
{
