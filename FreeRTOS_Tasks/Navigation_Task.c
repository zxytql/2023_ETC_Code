...
void Navigation_Task_Entry(void const * argument)
{
  ...
  for(;;)
  {
		...
		nav_vel.vx = fuzzypid_cal(&fzy_pid_x,Get_Now_X_Speed() * 1000.0f,PID_nav.x_set, PID_nav.x_now);
		nav_vel.vy = fuzzypid_cal(&fzy_pid_y,Get_Now_Y_Speed() * 1000.0f,PID_nav.y_set, PID_nav.y_now);
		nav_vel.ang_w = Navigation_Angle_Ctrl(PID_nav.yaw_now, 90.0f, 2.0f, 1.0f);
    osDelay(5);
  }
...
void PID_Nav_Init(PID_nav_t *pid_nav, fzy_pid_t *fzy_ptr_x, fzy_pid_t *fzy_ptr_y, fzy_pid_t *fzy_ptr_w, fzy_pid_t *fzy_ptr_v)
{
    ...
    fuzzy_init(fzy_ptr_x,-250.0f, 250.0f, -100.0f, 100.0f, 15.0f / 1000.0f, 18.0f / 1000.0f, 0.0f,0.0f,  12.0f / 1000.0f,15.0f / 1000.0f, 0.0f, 2000.0f / 1000.0f);
    fuzzy_init(fzy_ptr_w,-250.0f, 250.0f, -100.0f, 100.0f, 15.0f, 18.0f, 1.0f,2.0f,  12.0f,15.0f, 10.0f, 50.0f); //航向角不需要转换倍率
    fuzzy_init(fzy_ptr_y,-250.0f, 250.0f, -100.0f, 100.0f, 15.0f / 1000.0f, 18.0f / 1000.0f, 0.0f,0.0f,  12.0f / 1000.0f,15.0f / 1000.0f, 0.0f, 2000.0f / 1000.0f);    
