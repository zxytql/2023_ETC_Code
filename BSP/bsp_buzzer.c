/**
  ...
  * 2. 需要开启对应PWM以产生方波，并且只能在FreeRTOS任务中调用
  * @history
  *  Version    Date            Author           Modification
  *  V1.0.0     2022-03-11       zxy            First version
  *  V1.0.1     2022-04-10       zxy            添加歌曲小星星和警报声
	*  V1.1.0     2023-08-20       zxy            修改HAL_Delay(ms)为osDelay(ms)
  * @verbatim
  *
  ****************************(C) COPYRIGHT 2022 HCRT****************************
  */
...
void ActionDoneBuzzer(void)
{
    ...
    for(int i=0; i<(length/2); i++)//取数组数据
    {
        ...
        osDelay(4*sound[2*i+1]);//音长的时间都乘以5即一拍为500微秒，此值"5"可调整，只是播放的整个快慢而已，有点类似于视频快进和后退
    }
		buzzer_off();
...
void happy_time(void)
{
    ...
    for(int i=0; i<(length/2); i++)//取数组数据
    {
        ...
        osDelay(5*happy_birthday[2*i+1]);//音长的时间都乘以5即一拍为500微秒，此值"5"可调整，只是播放的整个快慢而已，有点类似于视频快进和后退
    }
}
